...
public class TrainPredictTask extends Task<Void> {
	...
	public TrainPredictTask(MainApplication mainApplication, String path,
			Integer percentage) {
	...
	protected Void call() {
		try {
			...
			Utils.getFilesByPercentage(percentage, filesToAnalize,
					filesToPredict);
			...
			for (File f : filesToPredict) {
				...
				Integer perc = (int) Math
						.round((cont.doubleValue() / nPredictDocuments) * 100);
				updateMessage("Realizando la predicción... " + perc + "%");
				updateProgress(cont.doubleValue(),
